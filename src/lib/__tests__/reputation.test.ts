...
describe("reputation hooks", () => {
  ...
  it("onPaymentSent submits economic transaction action with value", async () => {
    onPaymentSent("did:key:z6MkPayer", "pay-123", 25, { gig_id: "gig-1" });
    await vi.waitFor(() => expect(mockFetch).toHaveBeenCalled());
    const body = JSON.parse(mockFetch.mock.calls[0][1].body);
    expect(body.action_type).toBe("payment_sent");
    expect(body.action_category).toBe("economic.transaction");
    expect(body.agent_did).toBe("did:key:z6MkPayer");
    expect(body.value_usd).toBe(25);
    expect(body.metadata.payment_id).toBe("pay-123");
    expect(body.metadata.gig_id).toBe("gig-1");
  });

  it("onPaymentReceived submits economic transaction action with value", async () => {
    onPaymentReceived("did:key:z6MkReceiver", "pay-123", 25, { bounty_id: "bounty-1" });
    await vi.waitFor(() => expect(mockFetch).toHaveBeenCalled());
    const body = JSON.parse(mockFetch.mock.calls[0][1].body);
    expect(body.action_type).toBe("payment_received");
    expect(body.action_category).toBe("economic.transaction");
    expect(body.agent_did).toBe("did:key:z6MkReceiver");
    expect(body.value_usd).toBe(25);
    expect(body.metadata.payment_id).toBe("pay-123");
    expect(body.metadata.bounty_id).toBe("bounty-1");
  });
